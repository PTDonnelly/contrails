...
def read_ice_data(base_path, temperature_range, radius_range):
    ...
    num_columns, num_rows = read_dat_file(os.path.join(base_path, f"contrail_experiment\\aerosols_con1601.dat")).shape
    ...
    for it, temperature in enumerate(temperature_range):
        for ir, radius in enumerate(radius_range):
            filename = f"contrail_experiment\\aerosols_con{int(temperature/10):02d}{radius:02}.dat"
...
for i in range(nplots):
    if i == 0:
        ...
        for radius, color in zip(radius_range, particle_distribution_color_values):
            psd, D = build_psd(1.5, 2*radius, 6*radius, 15)
        ...
        for itemperature in range(n_temperatures):
            for iradius, radius in enumerate(radius_range):
                ...
                linestyle = temperature_linestyles[itemperature]
                ...
                ax.plot(ice_data[0, :, itemperature, iradius], data, ls=linestyle, color=color, alpha=0.75)
...
leg_ax = plt.subplot(gs[-2])
handles, labels = plt.gca().get_legend_handles_labels()
solid_line = Line2D([0], [0], label=r'$T$ = 230 K', color='k', ls='-')
dotted_line = Line2D([0], [0], label=r'$T$ = 160 K', color='k', ls=':')
handles.extend([solid_line, dotted_line])
leg_ax.legend(handles=handles, loc='lower right', fontsize=7)
