...
def main():
    ...
    # Define training and testing sets
    combined_df = read_and_combine_daily_data(f"{base_path}iasi/binned_olr/", f"{base_path}era5/daily_combined/", start_date, end_date)
    era5_fields = get_era5_fields(combined_df)
    X, y = split_data(combined_df, era5_fields)
    X_categorised = categorise_data(X, y, category='OLR_mean')

    # Convert true data values to Z-score values
    X_scaled = standardise_data(X_categorised)
